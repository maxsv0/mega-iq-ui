pipeline {
    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                script {
                    sh "gcloud beta compute --project=megaiq637 instances create-with-container ${deploymentName} --zone=europe-west2-c --machine-type=g1-small --network=default --network-tier=PREMIUM --metadata=google-logging-enabled=true --maintenance-policy=MIGRATE --tags=http-server,https-server --image-project=cos-cloud --boot-disk-size=10GB --boot-disk-type=pd-standard --boot-disk-device-name=${deploymentName} --container-image=${registryHost}/${registryName}:${containerTag} --container-restart-policy=always --scopes=https://www.googleapis.com/auth/devstorage.read_only,https://www.googleapis.com/auth/logging.write,https://www.googleapis.com/auth/monitoring.write,https://www.googleapis.com/auth/servicecontrol,https://www.googleapis.com/auth/service.management.readonly,https://www.googleapis.com/auth/trace.append"
                }
            }
        }
    }
